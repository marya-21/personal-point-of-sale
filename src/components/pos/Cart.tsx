    ...
    <div className="flex flex-col gap-3 py-3 border-b border-n-100">
      <div className="flex items-center gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-body font-semibold text-n-900 truncate">{item.name} — {item.unitName}</p>
          <p className="text-caption text-n-400">{formatRupiah(item.price_sell)}</p>
        </div>
        <Button
          onClick={() => removeItem(item.productId, item.unitId)}
          variant="ghost"
          size="icon"
          className="rounded-full hover:text-danger hover:bg-danger-bg"
        >
          <Trash />
        </Button>
      </div>
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Button
            ...
            variant="ghost"
            size="icon"
            className="rounded-full hover:bg-n-100"
            title="Kurangi"
          >
            <CircleMinus />
          </Button>
          <span className="w-6 text-center text-caption font-bold text-n-600">{item.qty}</span>
          <Button
        ...
        <div className="text-right min-w-[80px]">
          <p className="text-body font-bold text-n-900">
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="border-t border-n-200 pt-4 mt-2">
        <div className="flex justify-between items-center mb-4">
          <span className="text-body font-bold text-n-900">Total</span>
          <span className="text-title font-bold text-n-900">{formatRupiah(total)}</span>
        </div>
        <Button
          variant="primary"
