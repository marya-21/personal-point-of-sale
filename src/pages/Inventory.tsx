      ...
      <div className="bg-n-0 rounded-md shadow-sm p-4 border border-border">
          ...
          <ol className="space-y-2">
            {preview.map((item, i) => (
              <li key={item.name} className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-accent-100 text-accent-600 text-caption font-bold flex items-center justify-center shrink-0">
                  {i + 1}
                </span>
                <span className="text-body text-n-800 flex-1 truncate">
                  {item.name}
                </span>
        ...
        {items.length > 4 && (
          <button
            ...
            className="mt-3 text-caption text-accent-600 hover:underline"
          >
            See more →
          </button>
        )}
        ...
        <DialogContent className="max-w-md">
          ...
          <ol className="space-y-2 max-h-96 overflow-y-auto pr-1">
            {items.map((item, i) => (
              <li
                key={item.name}
                className="flex items-center gap-3 py-1 border-b border-n-100 last:border-0"
              >
                <span className="w-6 h-6 rounded-full bg-accent-100 text-accent-600 text-xs font-bold flex items-center justify-center shrink-0">
                  {i + 1}
                </span>
                <span className="text-sm text-n-800 flex-1">{item.name}</span>
                <span className="text-sm font-semibold text-n-500">
                  {item.qty} terjual
                </span>
              </li>
            ))}
    ...
    <>
      ...
      <Dialog open={showAll} onOpenChange={() => setShowAll(false)}>
        <DialogContent className="max-w-md">
          ...
          {outOfStock.length > 0 && (
            <div className="mb-4">
              <p className="text-xs font-semibold uppercase text-danger mb-2 tracking-wider">
                Habis
              </p>
              <ul className="space-y-2">
                {outOfStock.map((item) => (
                  ...
                  >
                    ...
                    <span className="text-sm text-n-800 flex-1">
                      {item.name}
                    </span>
    ...
    <div className="height-screen bg-n-100 p-8">
      <div className="mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-caption font-semibold text-n-400 mt-1">
                  ...
                  <TableBody>
                    {filtered.map((product) => (
                      <TableRow key={product.id}>
                        <TableCell>
                          <p className="text-body font-semibold text-n-900">
                            {product.name}
                          </p>
                        </TableCell>
                        <TableCell className="text-right">
                          <span
                            className={`text-body font-semibold ${product.stock === 0
                              ? "text-danger"
                        ...
                        <TableCell className="text-right">
                          <span className="text-body font-semibold text-n-900">
                            {formatRupiah(product.price_sell_base)}
                          </span>
                        </TableCell>
                        <TableCell className="text-right">
                          <span className="text-body text-n-700">
